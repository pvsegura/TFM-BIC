...
import type { VocabularyDb } from "../client.js";
import * as vocabularySchema from "../schema.js";

const here = path.dirname(fileURLToPath(import.meta.url));
const dataSrc = path.join(here, "..", "..", "..");
const identityMigrationsFolder = path.join(dataSrc, "identity", "db", "migrations");
const vocabularyMigrationsFolder = path.join(here, "..", "migrations");

/** Must match `migrations.table` in drizzle.vocabulary.config.ts. */
const VOCABULARY_MIGRATIONS_TABLE = "__drizzle_migrations_vocabulary";

export interface VocabularyTestDbHandle {
  db: VocabularyDb;
  /** The same database instance viewed as Identity's handle — lets a NODE_ENV=test composition
   * root build auth and vocabulary repositories over one shared database (`user_vocabulary` has
   * a foreign key to `users`). */
  identityDb: IdentityDb;
  /** Inserts a real `users` row and returns its id — `user_vocabulary` has a foreign key to
   ...
   * exercise `ON DELETE CASCADE`, or inserting a value the application would never write to
   * prove a CHECK constraint). Test-only. */
  rawExecute: (query: SQL) => Promise<unknown>;
  close: () => Promise<void>;
}

/**
 * A real (WASM-compiled) Postgres with Identity's and Vocabulary's migrations applied in
 * dependency order — Identity first (the FK target `users` must exist), each set tracked in its
 * own table exactly as `drizzle-kit migrate` does. Same approach as the other contexts'
 * test-support — see docs/adr/adr-005-database.md. Test-only.
 */
export async function createVocabularyTestDb(): Promise<VocabularyTestDbHandle> {
  const client = new PGlite();
  const pgliteDb = drizzle(client, {
    schema: { ...identitySchema, ...vocabularySchema },
  });

  await migrate(pgliteDb, { migrationsFolder: identityMigrationsFolder });
  await migrate(pgliteDb, {
    migrationsFolder: vocabularyMigrationsFolder,
    migrationsTable: VOCABULARY_MIGRATIONS_TABLE,
  });
  ...
  return {
    ...
    db: pgliteDb as unknown as VocabularyDb,
    ...
    reset: async () => {
      await pgliteDb.execute(
        sql`TRUNCATE TABLE "user_vocabulary", "users" RESTART IDENTITY CASCADE;`,
      );
    },
